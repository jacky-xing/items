<template>
	<div class="car">
		...
		<div class="car_body" id="car_b" v-for="(value,idx) in data">
			<img :src="value.img">
					...
					if(res.data.status) {
						...
						http.get('selgenerateorder').then((res) => {
							if(res.data.status) {
								...
								this.total -= e.target.parentNode.nextElementSibling.nextElementSibling.children[0].children[0].innerText*1;
							} else {
								router.push('login')
							}
						});
